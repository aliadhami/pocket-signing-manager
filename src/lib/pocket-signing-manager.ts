...
export class PocketSigner implements PolkadotSigner {
  ...
  public setNodeUrl(url: string): void { 
    this.nodeUrl = url;
    if (url.includes('testnet')) { this.setNetwork('testnet'); } 
    else if (url.includes('mainnet')) { this.setNetwork('mainnet'); }
  }
  ...
  private async waitForSignature(reqId: string): Promise<HexString> {
    return new Promise<HexString>((resolve, reject) => {
      this.checkHistoryForSignature(reqId, resolve, reject).catch(err => {
        console.error(`[ERROR] Initial history check failed: ${(err as Error).message}`);
      });
      const intervalId = setInterval(() => {
        this.checkHistoryForSignature(reqId, resolve, reject).catch(err => {
          console.error(`[ERROR] History check failed: ${(err as Error).message}`);
        });
      }, POLL);
    ...
    if (historyResp.success && Array.isArray(historyResp.request_history)) {
      for (const request of historyResp.request_history) {
        if (request.req_id === reqId && (!request.network || request.network === this.network)) {
          ...
          if (!pendingReq || pendingReq.resolved) { return; }
  ...
  private async startPolling(): Promise<void> {
    ...
    const poll = async () => {
      if (!this.pollingActive || this.pendingRequests.size === 0) { this.pollingActive = false; return; }
      try {
        const historyResp = await call<RequestHistoryResponse>('get_request_history', {
          session_id: this.sid,
          network: this.network
        ...
        if (historyResp.success && Array.isArray(historyResp.request_history)) {
          for (const request of historyResp.request_history) {
            if (this.processedSignatures.has(request.req_id)) { continue; }
            if (request.network && request.network !== this.network) { continue; }
        ...
        if (this.processedSignatures.size > 100) {
          ...
          for (const item of oldItems) { this.processedSignatures.delete(item); }
        }
        const now = Date.now();
      ...
      if (this.pendingRequests.size > 0) { setTimeout(poll, POLL); } 
      else { this.pollingActive = false; }
    };
    poll();
  }
}
...
export class PocketSigningManager implements SigningManager {
  ...
  public static async create(args: { appName: string; network?: string; ss58Format?: number; }): Promise<PocketSigningManager> {
    const { appName, network = 'testnet', ss58Format } = args;
    const signingManager = new PocketSigningManager(appName);
    if (network) { signingManager.setNetwork(network); }
    if (ss58Format) { signingManager.setSs58Format(ss58Format); }
  ...
  constructor(private readonly appName: string) {
    ...
    if (storedSession && storedSession.appName === this.appName) {
      ...
      if (storedSession.network) { this.currentNetwork = storedSession.network; }
  ...
  public setNetwork(network: string): void {
    if (network !== 'testnet' && network !== 'mainnet') { throw new Error(`Invalid network: ${network}. Must be 'testnet' or 'mainnet'`); }
    this.currentNetwork = network;
    this.externalSigner.setNetwork(network);
    if (this.sessionCreated) { call('update_session', { session_id: this.sid, network: this.currentNetwork }).catch(e => console.error('Failed to update session network:', e)); }
  }

  public setSs58Format(f: number): void { this.ss58 = f; }

  private ss58OrThrow(m: string): number {
    if (this.ss58 === undefined) { throw new Error(`Call setSs58Format before ${m}`); }
    return this.ss58;
  }
      ...
      if (storedSession && storedSession.appName === this.appName && storedSession.sid === this.sid) {
        try {
          ...
          if (resp.session) {
            ...
            if (this.currentNetwork === 'testnet' && Array.isArray(row.testnet_wallets) && row.testnet_wallets.length) { networkWallets = row.testnet_wallets; } 
            else if (this.currentNetwork === 'mainnet' && Array.isArray(row.mainnet_wallets) && row.mainnet_wallets.length) { networkWallets = row.mainnet_wallets; } 
              ...
              await call('update_session', { session_id: this.sid, [walletField]: networkWallets });
            }
            ...
            if (!this.sessionCreated) { await this.bootstrap(); }
          }
        } catch (err) {
  ...
  public getExternalSigner(): PocketSigner { return this.externalSigner; }

  public async getLocalKeys() { return this.wallets.map(w => ({ name: 'wallet', publicKey: w, address: w })); }
      ...
      try {
        ...
        if (resp.success && resp.session) {
          ...
          if (row.network !== detectedNetwork) {
            await call('update_session', { session_id: this.sid, network: detectedNetwork });
  ...
  public __pocketRegisterSdk(sdk: Polymesh): void { this.sdk = sdk; }
  ...
  private async reconnectSession(): Promise<void> {
    ...
    try {
      await call('create_session', { session_id: this.sid, origin_hash: 'cli', network: this.currentNetwork });
      this.sessionCreated = true;
    } catch (error) {
  ...
  private async waitForConnection(): Promise<void> {
    ...
    try {
      while (Date.now() - started < TIMEOUT) {
        if (cancelled) { throw new Error('User cancelled the connection process.'); }
        try {
          const resp = await call<SessionResponse>('get_session', { session_id: this.sid });
          if (resp.session) {
            const row = parseSessionRow(resp.session);
